...
def get_withdraw_total(loginUid, loginSid):
    url = 'https://integralapi.kuwo.cn/api/v1/online/sign/v1/withdrawDetails'
    params = {
        'loginUid': loginUid,
        'loginSid': loginSid,
...
class KuwoManagerPlugin(Star):
    def __init__(self, context: Context, config: dict = None):
        ...
        # ---------- 并发锁 ----------
        self._update_lock = asyncio.Lock()  # 新增：保证更新 CODE 的原子性
    ...
    # 以下管理员子操作与之前完全相同，为节省篇幅此处省略，但实际必须保留。
    # 请确保您已包含所有 _admin_bind_select_phone 等方法。
    # 如果您的文件缺少这些方法，请从上一版本复制。
    # 为了完整性，建议用您已有的完整文件替换。
